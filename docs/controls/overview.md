...
Flet UI is built of controls. Controls are organized into hierarchy, or a tree, where each control has a parent (except [Page](page)) and container controls like [Column](column), [Dropdown](dropdown) can contain child controls, for example:
...
Effective inside [`Stack`](stack) only. The distance that the child's bottom edge is inset from the bottom of the stack.
...
Effective inside [`Stack`](stack) only. The distance that the child's left edge is inset from the left of the stack.

### `right`

Effective inside [`Stack`](stack) only. The distance that the child's right edge is inset from the right of the stack.

### `top`

Effective inside [`Stack`](stack) only. The distance that the child's top edge is inset from the top of the stack.
